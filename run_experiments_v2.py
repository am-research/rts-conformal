...
from cp_methods import (
    cp_pwcet, empirical_quantile,
    empirical_coverage, coverage_shortfall, bound_efficiency,
    run_coverage_trial, AdaptiveCI, cqr_calibrate, cqr_predict
)
from evt_compat import evt_gpd, describe as _evt_describe
import cp_methods as _cpm
_cpm.evt_gpd = evt_gpd   # run_coverage_trial resolves evt_gpd in cp_methods'
                         # own namespace, so importing the shim here is not
                         # enough -- the name must be rebound in that module.
...
if __name__ == '__main__':
    ...
    def _save(tag=''):
        """Write results after every stage, so a failure late in the run does
        not discard the stages that already succeeded."""
        import os
        os.makedirs('results', exist_ok=True)
        ...
        if tag:
            print(f"  [saved results/results.json after {tag}]")

    for _name, _fn in [('exp1_exp2', run_exp1_exp2),
                       ('exp3', run_exp3),
                       ('exp_smalln', run_exp_smalln),
                       ('exp4', run_exp4),
                       ('exp5', run_exp5),
                       ('summary_table', run_summary_table)]:
        try:
            _fn()
            _save(_name)
        except Exception as _e:
            import traceback
            print(f"\n!! {_name} FAILED: {type(_e).__name__}: {_e}")
            traceback.print_exc()
            _save(_name + ' (failed)')
